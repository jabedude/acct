...
impl AcctV3 {
    ...
    /// Set command name
    pub fn set_command(&mut self, new: String) -> Result<(), ()> {
        if new.len() > 15 {
            return Err(());
        }
        ...
        for (count, b) in c.into_bytes().iter().enumerate() {
            self.inner.ac_comm[count] = *b;
        }

        for byte in self.command.len()..16 {
            self.inner.ac_comm[byte] = b'\0';
        }
